# Selecting several games at once

You can act on several games in one step: add or remove favorites, hide or unhide, change category, or remove from the library. Selection works the way it does in File Explorer and only exists in the Games Library. The Scan for Games and Add Folder dialogs already have their own checkboxes for choosing what to import.

## Selecting

- Ctrl+click a game card to add it to the selection, or to take it back out.
- Shift+click selects every card between the last one you clicked and this one.
- Ctrl+A anywhere in the library (outside the search box) selects every visible game.
- Selected cards get a blue border, and a selection bar appears over the bottom of the library with the count and the common actions. This works the same in every view: posters, extra-large posters, compact icons, and the details list.

## Acting on the selection

- The selection bar has Add to (or Remove from) Favorites, Change Category, and Hide (or Unhide) as buttons, More for everything else, and Clear to drop the selection. Tab reaches it like any other button. More opens the same menu as a right-click.
...
- Click anywhere that is not a card (empty library space, the toolbar, the sidebar), press Escape, or plain-click a card (which also opens its details as usual).
- Changing the search text or the category tab clears the selection as well, so a game you can no longer see is never changed by mistake. Clicking into the search box on its own keeps the selection, the same as File Explorer.
